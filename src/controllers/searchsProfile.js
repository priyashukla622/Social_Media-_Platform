  ...
  const searchProfile = async (req, res) => {
    try {
      ...
      if (username) {
        ...
        return res.status(200).json({ message: "Posts found successfully", profiles: profiles });
      } 
      else {
        profiles = await Model.profileModel.find({});
        if (profiles) {
          return res.status(200).json({ message: "All posts found successfully", profiles: profiles });
